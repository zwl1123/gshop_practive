<template>
  <div class="shop-info">
    <div class="info-content">
      <section class="section"><h3 class="section-title">配送信息</h3>
        <div class="delivery">
          <div>
            <span class="delivery-icon">{{info.description}}</span>
            <span>由商家配送提供配送，约 {{info.deliveryTime}} 分钟送达，距离 {{info.distance}}</span></div>
      ...
      <section class="section">
        ...
        <div class="activity" >
          <div class="activity-item activity-green"  v-for="(support,index) in info.supports"
               :key="index" :class="activityClasses[support.type]">
            <span class="content-tag">
              <span class="mini-tag">{{support.name}}</span>
            </span>
            <span class="activity-content">{{support.content}}</span></div>
        </div>
      </section>
      <div class="split"></div>
      <section class="section">
...
<script>
  ...
  export default {
    ...
    methods:{
      setUlWidth () {
        const ul = this.$refs.picList
        const liWidth = 120
        const space = 6
        ul.style.width = this.info.pics.length * (liWidth + space) - space+'px'
      },
      _initScroll(){
