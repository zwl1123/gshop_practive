    ...
    <div class="ratings-content">
      <div class="overview">
        ...
        <div class="overview-right">
          ...
          <div class="delivery-wrapper"><span class="title">送达时间</span> <span class="delivery">{{info.deliveryTime}} 分钟</span></div>
        </div>
      </div>
      <div class="split"></div>
      ...
      <div class="rating-wrapper">
        <ul>
          <li class="rating-item" v-for="(rating,index) in filterRating" :key="index" >
            <div class="avatar"><img width="28" height="28"
  ...
  export default {
    ...
    methods:{
      setSelectType(selectType){
        ...
        //解决切换selectType，滚动条不够或太长的问题， 让滚动条自动刷新
        this.$nextTick(() => {
          this.scroll.refresh()
        })
      },
    ...
    computed:{
      ...
      positiveCount(){
        const {ratings} = this
        const arrs = ratings.filter((rating,index)=>{
      ...
      filterRating(){
        ...
        this.arrs = this.ratings.filter((rating,index)=>{
          //看rating的text和rateType
          const {rateType,text} = rating
          if(selectType === 2){
            return  !OnlyContent || text.length>0
          } else {
            return  rating.rateType === selectType && (!OnlyContent || text.length>0)
          }
        })
        return this.arrs
      }
